...
public class Puzzle2 extends Puzzle implements Comparable {
	...
	public Puzzle2(int n) {
		...
		initialisation();
		System.out.println("Difficulté : "+this.inversion());
		//print();
		/*
		 * for (int i=0;i<n;i++) { for(int j=0;j<n;j++) {
		 * System.out.println(tabS[i][j]);
		 * 
		 * 
		 * } }
		 */
		  //while(!realisable()) { init2(); }
	...
	public Puzzle2(int size_enter, int[][] valeurs) {
		this.n = size_enter;
		this.tab =new int[n][n];
		this.tabS =new int[n][n];
		this.init();
	...
	public int gaschnig() {
		...
		while(!this.isSuccess()) {
			...
			if(x==xx&&y==yy) {
				for (int i=0;i<n;i++) {
					for(int j=0;j<n;j++) {
						if(tab[i][j]!=tabS[i][j] && !play&& (tab[i][j]!=-1) ) {
							//System.out.println(tab[i][j]+"/"+tabS[i][j]);
							tab[x][y]=tab[i][j];
							tab[i][j]=-1;
							play = true;
