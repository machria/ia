...
public class App {
	...
	public static void main(String[] args) throws Exception {
		...
		//BD
				...
				for (int i = 0; i < a.getListPuzzle().size(); i++) {
					...
					if(dfs==null)
						...
						App.saveSolutionToFile(a.getListPuzzle().get(i), dfs.getTruePlat(), "DFS",dfs.getCompMem(),dfs.getCompTemp(),duration);
				}
				//IDS
				int limite=0;
				for (int i = 0; i < a.getListPuzzle().size(); i++) {
					...
					if(ids==null)
						...
						limite = ids.getCo();
						App.saveSolutionToFile(a.getListPuzzle().get(i), ids.getEnd(), "IDS",ids.getCompMem(),ids.getCompTemp(),duration);
					}
				...
				for (int i = 0; i < a.getListPuzzle().size(); i++) {
					...
					App.saveSolutionToFile(a.getListPuzzle().get(i), dls.getTruePlat(), "DLS",dls.getCompMem(),dls.getCompTemp(),duration);
				}
		//GFS
		for (int i = 0; i < a.getListPuzzle().size(); i++) {
		...
		for (int i = 0; i < a.getListPuzzle().size(); i++) {
			...
			App.saveSolutionToFile(a.getListPuzzle().get(i), bfs.getTruePlat(), "BFS",bfs.getCompMem(),bfs.getCompTemp(),duration);
		}
		//ILS
		for (int i = 0; i < a.getListPuzzle().size(); i++) {
